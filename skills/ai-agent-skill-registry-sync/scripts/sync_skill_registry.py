...
def render_central_page(collected: dict[str, tuple[AgentSource, list[SkillEntry]]], today: str) -> str:
    ...
    parts.append(
        f"- [[domains/AI Agent工程/90-Skill注册表/{PERSONAL_REGISTRY_OUTPUT.removesuffix('.md')}|个人/项目 Skill 注册库]]："
    ...
    for source, entries in collected.values():
        ...
        parts.append(f"- [[domains/AI Agent工程/90-Skill注册表/{target}|{source.label} Skill 注册页]]：{len(entries)} 个 skill。来源：{roots}\n")
    parts.append("\n## 全局能力索引\n\n")
    by_category: dict[str, list[SkillEntry]] = {}
    for entry in all_entries:
        by_category.setdefault(entry.category, []).append(entry)
...
def render_personal_page(collected: dict[str, tuple[AgentSource, list[SkillEntry]]], today: str) -> str:
    ...
    parts.append(
        ...
        "这些仍保留在 [[domains/AI Agent工程/90-Skill注册表/02-跨Agent Skill注册库|跨 Agent Skill 注册库]]。\n\n"
    )
...
def prepend_log(wiki_root: Path, today: str, collected: dict[str, tuple[AgentSource, list[SkillEntry]]], touched: list[Path]) -> Path | None:
    ...
    entry = (
        ...
        f"- Updated: `domains/AI Agent工程/90-Skill注册表/02-跨Agent Skill注册库.md`\n"
        f"- Updated: `domains/AI Agent工程/90-Skill注册表/01-个人与项目Skill注册库.md`\n"
        f"- Updated: agent-specific skill registry pages under `domains/AI Agent工程/90-Skill注册表/`\n"
        f"- Updated: `index.md`\n"
        f"- Updated: `domains/AI Agent工程/index.md`\n"
